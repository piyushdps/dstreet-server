...
const pusher = new Pusher({
  appId: "1283995",
  key: "f4c8722152b206bd3a33",
  secret: "4a4151743f2ae91cca43",
  cluster: "ap2",
  useTLS: true
});
...
router.put("/:id", auth, async (req, res) => {
  try {
    if (req.body.status !== "USER_CANCELLED" && currentOrder.status !== "PREPARING")
      throw {
        message:
          "Non-admin user can only cancel their order, that too, only if it is not being prepared.",
      };
    const currentOrder = await Order.findById(req.params.id);
    if (currentOrder.status !== "PENDING" )
      throw { message: "Can only cancel order if its not preparing." };
      pusher.trigger("my-channel", "my-event", {
        message: "hello world"})
